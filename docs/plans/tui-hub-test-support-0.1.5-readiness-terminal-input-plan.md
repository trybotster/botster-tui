...
- Extend the isolated live-hub smoke so it waits until the TUI observes `Attached`, forwards a unique marker through `TuiApp::handle_dispatch(InputDispatch::TerminalForward { ... })`, and then observes that marker in PTY echo/output through the TUI's normal drain/render state.
- If that live proof fails, make the smallest correction to `attached_session`/input ownership in the existing `TuiApp` response and dispatch path, with a focused regression test. Do not add a parallel input state machine.
...
- Regression evidence must show the production entry point remains `run_loop -> InputRouter::dispatch_event -> TuiApp::handle_dispatch -> DaemonRequest::SendInput`; fixture-only state mutation is insufficient.
