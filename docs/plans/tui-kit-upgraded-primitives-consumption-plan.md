...
- Route delivered plugin surface `UiNode` rendering through the kit path already used by production: `DaemonResponseKind::PluginSurface` -> `TuiApp::plugin_surface` -> `plugin_surface_body_node`/`UiNode` validation -> `renderer::tui_capabilities()` -> `renderer::render_node` or `renderer::render_to_lines`.
...
- Update README manual live-runtime instructions if the upgraded fixture adds a new required launch/open step or a new visible capability/diagnostic expectation.
...
- Assumption: the existing live-runtime panel is the correct host for plugin-surface render/action evidence; do not add a new screen unless the public DTO requires a different route/open shape.
...
- Docs: README manual live-runtime instructions if visible behavior or launch steps change, plus this plan artifact.
...
- `crates/botster-tui/src/app.rs`
  - Keep production rendering and action paths wired through `TuiApp`; add/adjust tests for plugin surface rendering, keyboard/mouse selection, action dispatch, and action feedback.
...
- `README.md`
  - Update manual live-runtime docs if new composite fixture/manual verification is added.
...
5. Add or extend a production-path test that applies a `DaemonPluginSurface` response containing the composite surface to `TuiApp`, calls `surface()`, and renders with `renderer::render_to_lines`. This proves the actual TUI path consumes the upgraded primitives.
6. Run the existing terminal/session tests to prove no regression in `TerminalForward`, resize, attach, drain, and headless live-runtime paths.
...
- False-green risk: a pure `render_to_lines` fixture test would not prove plugin surfaces use the new behavior. Mitigation: include a `TuiApp::apply_response` production-path test with a composite `DaemonPluginSurface`.
...
- Layout density risk: composite application screens may exceed the existing live-runtime panel size. Mitigation: use deterministic `render_to_lines` dimensions in tests and avoid new navigation unless required by public DTO shape.
...
- Required assertions:
  ...
  - A `TuiApp` plugin-surface response carrying the composite surface renders through `TuiApp::surface()` and the kit-backed renderer.
  ...
  - Existing terminal/session behavior still passes, especially attach, terminal input forwarding, resize, drain/snapshot/scrollback rendering, and headless live-runtime.
  - Manual docs explain how to launch the TUI and open a plugin app/surface using the new primitives if the command differs from the existing README live-runtime path.
