...
- Repo context: `botster-tui` is a Rust TUI client over hub contracts and `botster-tui-kit`; production path is `TuiApp::refresh_read_models()` -> hub `DaemonRequest`s -> `TuiApp::surface()` -> shared UiNode renderer. Existing tests already cover package/app route DTO display, plugin surface render/action DTOs, invalid UiNode diagnostics, and live hub plugin contract matrix smoke.
...
- Replace TUI plugin navigation discovery with hub-admitted navigation DTO consumption. The TUI should request the registry from the hub, store the returned navigation entries in `TuiApp`, render them as client presentation rows, and open plugin routes from those entries.
...
- `README.md`: update local hub live-runtime section if behavior or command evidence changes.
...
2. Add `TuiApp` state for admitted navigation entries and refresh it from the new hub request during `refresh_read_models()`. Keep `ListApps`/`ListPackages` for status, lifecycle, and package management, but stop using package routes as the plugin navigation source of truth.
...
- Manual evidence for production path: identify in implementation report which `TuiApp` refresh/open action calls the new hub request and which rendered node shows the selected plugin surface.
