...
## Plan Review corrections — fourth review (findings 1787245559_*)

| Finding | Class | Fix in this revision |
| --- | --- | --- |
| `finding_1787245559_832380` active context used an undefined "newest" row | product / high | Confirmed, and it is the same defect class already corrected for the attention band. `session_request` rows carry no `created_at`, sequence, or order field, the store iterates a `BTreeMap` by row id, and PP has **no uniqueness guard on `session_id`** (verified absent at `cd7c2f9`) while Botster can reuse one agent session across run steps. Scope 6 now derives only the active `run_id`, by collecting every row matching the focused `session_id`, reducing to distinct `run_id` values, and accepting exactly one; zero rows or two-or-more distinct runs fail closed with a bounded diagnostic and no guess. `step_id` and `ticket_id` are no longer derived at all, since run-only filtering does not need them. Check 7 adds both the agreeing-rows case and the disagreeing-rows case, asserted with rows inserted in both id orders so the outcome cannot depend on iteration order. New assumption A10 and risk R13. |
| `finding_1787245559_745456` stale revision-4 table row contradicted revision 5 | product / medium | Confirmed: the revision-4 facts table still told Implement that a provider failure yields `DaemonEntityFrame::Error` and pointed at A7 and check 7a, which revision 5 had already disproved in Scope 6a. That row now states the verified admission path (`OperatorError` → `DaemonTransportError::Protocol`, hub-client `lib.rs:362-366`) and reserves `DaemonEntityFrame::Error` for a genuine in-stream error, so the document no longer contains contradictory instructions. |
| `finding_1787245559_124775` notice duration was proposed, not decided | product / medium | `TRANSIENT_NOTICE_TTL` is now **decided at 5 s** with the reasoning recorded, and changing it requires a plan change rather than an Implement choice (A11). Check 8 replaces the constant-only assertion with a clock-controlled visible boundary: present just before the deadline, absent at and after it, through the production tick path. |
...
| Providers fail closed on incomplete family load | `d42ab56` | A provider failure never yields a partial snapshot. **Corrected in revision 5:** the failure surfaces at **subscribe admission** — Hub answers `SubscribeEntities` with `OperatorError`, and hub-client maps any non-`EntitySubscribed` response to `DaemonTransportError::Protocol` (`lib.rs:362-366`). It does **not** produce `DaemonEntityFrame::Error`, which remains reserved for a genuine in-stream error on an established subscription. Scope 6a defines the two paths and their separate retry handling; A7 and check 7a follow that split. |
...
6. **Active workflow context and production durable wiring.**
   ...
   **(6a) Two distinct failure modes, with a request bound on the always-on families.** Making these families always-on removes the surface-demand guard that previously limited retry attempts, so the retry policy is part of this change, not an afterthought.
   ...
   - **Active context derivation — deterministic, order-free, fail closed.** The only value this plan needs is the active `run_id`, because both the notice filter and the attention band key on run identity alone. Rule:
     1. Collect **every** `project-pipelines.session_request` row whose `session_id` equals the focused `selected_session`.
     2. Reduce those rows to their set of distinct `run_id` values.
     3. Exactly one distinct `run_id` → that is the active run.
     4. Zero matching rows → no context → suppress (A5).
     5. Two or more distinct `run_id` values → **ambiguous** → fail closed, suppress, record one bounded diagnostic. Never guess.
     - This never depends on iteration order. PP publishes no `created_at`, sequence, or order field on `session_request` rows, and `EntityOptionsStore` iterates a `BTreeMap` keyed by row id, so any "newest row" rule would be undefined — the same defect corrected for the attention band. PP also enforces **no uniqueness guard on `session_id`** (verified absent at `cd7c2f9`), and Botster can reuse one agent session across run steps, so several rows sharing a `session_id` is a legitimate shape. Step 3 handles that case correctly whenever those rows agree on the run, which is what same-run reuse produces; step 5 refuses to choose when they do not.
     - `step_id` and `ticket_id` are deliberately not derived. Run-only filtering needs neither, and deriving them would reintroduce an ordering question with no authoritative answer.
...
9. **Transient notice UI (app policy only).** `transient_notice: Option<TransientNotice { text, question_id, kind, deadline: Instant }>` with **`TRANSIENT_NOTICE_TTL = 5 s` (decided, not proposed)**, rendered as the `workspace-transient-notice` band, expired by deadline check in `poll_hub` (the ≤100 ms tick). Five seconds is long enough to read a ≤280-character notice and short enough that a stale notice cannot outlive the workflow moment it describes; the ≤100 ms tick makes the observed dismissal boundary 5.0–5.1 s. Changing this visible behavior requires a plan change, not an Implement choice. No TUI Kit change.
...
- A10: Ambiguous workflow context is rare but must be safe, not clever. When rows sharing the focused `session_id` disagree on `run_id`, showing a notice for either run would be a wrong-workflow notice, which is worse than showing none. Suppression is the correct outcome and is asserted rather than assumed. This also removes any dependence on PP guaranteeing global `session_id` uniqueness, which it does not.
- A11: `TRANSIENT_NOTICE_TTL = 5 s` is a decided product value, not an Implement default. Check 8 asserts the visible boundary so a silent change fails a test.
...
- R13: PP could later add a `session_id` uniqueness guarantee or an ordering field, which would permit a simpler exact-one rule. The distinct-run rule stays correct under that change, so no follow-up is required; Implement must not weaken it to "first row" or "newest row" on the assumption that duplicates cannot occur.
...
7. Workflow filter (run matching only), with production-shaped `session_request` rows carrying `run_id`, `step_id`, and `ticket_id`:
   ...
   - **several rows share the focused `session_id` and agree on one `run_id`** → resolve to that run and apply the rules above (the same-session-reuse case);
   - **several rows share the focused `session_id` but carry two or more distinct `run_id` values** → ambiguous, suppress, one bounded diagnostic, and never select either run. Assert this with rows inserted in both id orders so the result cannot depend on `BTreeMap` iteration.
...
8. Notice policy: matching `PackageEvent` sets one notice; a second matching event replaces it (latest wins); a payload without `notice` drops with a bounded diagnostic. TTL boundary, clock-controlled and never slept: the band is **present** just before the 5 s deadline and **absent** at and after it, asserted through the production tick path so the constant and the visible behavior are proven together.
