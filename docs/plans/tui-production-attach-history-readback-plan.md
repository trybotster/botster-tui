...
- Model one bounded attach/reconnect restoration cycle in `DogfoodApp`:
  - begin the cycle with a monotonic `Instant` deadline five seconds after the production attach request; five seconds mirrors the merged hub's `external_daemon_attach_replays_prior_history_with_renderable_byte_count` runtime test, which drains at 30 ms intervals until history or that deadline;
  ...
  - issue one `DaemonRequest::CaptureSnapshot` when hydration completes, whether by ordered history or deadline, and retain its typed metadata (`rows`, `cols`, optional `payload_format`, and `payload_bytes`) for observable status/diagnostic rendering without inventing access to opaque payload bytes;
  ...
  - guarantee the cycle terminates once on ordered history, deadline, or terminal lifecycle end, and reset its deadline/fallback state on detach, session exit, session switch, transport failure, and a new reconnect attach.
...
- TUI/client state and production path — `crates/botster-tui/src/app.rs`
  ...
  - add the five-second hydration deadline, completion state, separate read-screen fallback slot, and snapshot-metadata state;
...
- Focused Rust tests in `crates/botster-tui/src/app.rs` should prove:
  ...
  - attach and repeated drains preserve request ordering and the same `subscription_id`, with `ReadScreen` only after deadline and `CaptureSnapshot` after either ordered-history or deadline completion;
...
- Real runtime proof in the existing isolated-hub test:
  ...
  - add a second real-daemon attachment whose terminal buffer remains empty through the bounded window; prove the cycle reaches its finished state, exactly one `ReadScreen` and one `CaptureSnapshot` are observed, and non-empty returned screen text (when present) is shown by `renderer::render_to_lines`; if the real empty screen is empty, assert the honest empty fallback plus finished/request state and retain the synthetic non-empty render proof rather than fabricating text;
