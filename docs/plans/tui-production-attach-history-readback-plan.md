...
- Model one bounded attach/reconnect restoration cycle in `DogfoodApp`:
  ...
  - track which session owns the displayed terminal buffer. When attaching a different session id, clear `terminal_output`, the fallback slot, and session-scoped snapshot metadata before starting hydration; on same-session reconnect, preserve existing terminal/fallback content while starting a fresh bounded cycle;
  ...
  - guarantee the cycle terminates once on ordered history, deadline, or terminal lifecycle end, and reset its deadline state on detach, session exit, transport failure, and a new reconnect attach. Session switching additionally resets all session-owned terminal/readback presentation state as described above.
...
- Session-mixing risk: hydration/readback identity could accidentally depend on never-set `attached_session` or stale selection. Mitigation: bind every cycle/request to the session/subscription ids captured directly at attach and distinguish different-session clearing from same-session reconnect preservation.
- Cross-session buffer leak risk: the current flat terminal buffer is never cleared, so session A could suppress fallback and render under session B. Mitigation: assign the buffer an owner session id, clear terminal/fallback/snapshot state only on different-session attach, and preserve it on same-session reconnect.
...
- Focused Rust tests in `crates/botster-tui/src/app.rs` should prove:
  ...
  - different-session attach clears terminal output, fallback, and snapshot metadata before hydration; same-session reconnect preserves presentation content while resetting only cycle/deadline state;
