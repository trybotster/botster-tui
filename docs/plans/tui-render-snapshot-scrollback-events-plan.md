...
- Current code context: `TuiApp::apply_response` in `crates/botster-tui/src/app.rs` appends `DaemonEvent::TerminalOutput { data, .. }`, handles `ProcessExit` and `AttachState`, and ignores all other daemon events. The terminal renderer already consumes `TuiApp.terminal_output` through the `TerminalView` primitive.
...
- In `TuiApp::apply_response`, handle `DaemonEvent::Snapshot` and `DaemonEvent::Scrollback` beside `TerminalOutput`.
...
- Test harness: Rust unit tests and existing live isolated hub live-runtime script.
...
- No README/docs update is required unless implementation finds user-facing live-runtime behavior changed enough to document.
...
- Live-test trigger risk: isolated hub live-runtime may not force Snapshot/Scrollback variants. Mitigation: require direct event-fixture tests for DTO handling and keep live live-runtime for public-client runtime path health.
...
- Focused tests in `crates/botster-tui/src/app.rs`:
  ...
  - Existing `terminal_output_renders_as_terminal_primitive_content` or a nearby assertion proves history-appended output still renders through `live-runtime-terminal`, not the old text child hit target.
...
`run_loop` -> `TuiApp::poll_hub` / `request_and_apply` -> public `HubConnection::request` using `botster_hub_client::{write_frame, read_frame_from_reader}` -> `TuiApp::apply_response` -> `terminal_output` -> `TuiApp::surface` -> `renderer::render_node` for `UiNodeKind::TerminalView`.
